...
import json
import os, cv2, numpy, base64, subprocess, random
import psutil
import time
...
class LDPlayer:
    def __init__(self):
        self.pathLD = "D:\LDPlayer\LDPlayer9"
        self.local = ""
    ...
    def ExecuteLD(self, shell):
        return subprocess.Popen(f"ldconsole {shell}", creationflags=subprocess.CREATE_NO_WINDOW, stdout=subprocess.PIPE, shell=True, cwd=self.pathLD, start_new_session=True).stdout.read()
    def Connect(self):
        # for i in range(3):
        # check = subprocess.Popen(f"adb connect {self.local}", creationflags=subprocess.CREATE_NO_WINDOW, stdout=subprocess.PIPE, shell=True, cwd=self.pathLD, start_new_session=True).stdout.read()
        check = self.AdbLd("connect "+self.local)
        print(check)
        if "connected" not in str(check): return self.Connect()
        
    # def Connect2(self):
    #     self.AdbLd("connect 127.0.0.1")
    def Get(self):
        # print("lấy local")
        port = 5555
        id = int(self.NameOrId)
        for i in range(2):
            port += id
        self.local = "127.0.0.1:%s"%port
        # self.local = self.AdbLd("get-serialno")
        print(self.local)
    ...
    def DumXml(self):
        ...
        path = "\"\"%s\"\""%os.path.join(os.getcwd(), f"resources\\window_dump_{self.NameOrId}.xml")
        self.AdbLd(f"pull /sdcard/window_dump.xml {path}")
        return path.replace('"', '')
    def FindXml(self, element, file: os.path, index: int=0):
        try:
            tree = html.parse(file).xpath(element)
        ...
        for bound in tree:
            gg = bound.attrib['bounds'].split('][')[0].replace('[', '').split(',')
            pos.append(tuple([int(gg[0]), int(gg[1])]))
        return (pos, tree if tree == [] else tree[index])
    ...
    def PushImg(self, path):
        self.AdbLd(f"root")
        print(self.AdbLd(f"push {path} /sdcard/Download/2.png"))
        print(self.AdbLd("shell am broadcast -a android.intent.action.BOOT_COMPLETED -n com.android.providers.media/.MediaScannerReceiver"))
    ...
    def TapImagePath(self, bg, src: bytes, click=True):
        img = cv2.imdecode(np.frombuffer(src, dtype=np.uint8), cv2.IMREAD_COLOR)
        img2 = cv2.imread(bg)
        w, h = img.shape[1], img.shape[0]
        result = cv2.matchTemplate(img, img2, cv2.TM_CCOEFF_NORMED) 
        location = numpy.where(result >= 0.8)
        ...
        if is_match:
            x, y = data[0][0], data[0][1]  
            # print(x,y)5
            if click: self.Click(x + int(w/2), y + int(h/2))
            return (x, y)
        else:
            return (False, False)
    def TapImage(self, pathImg: os.path):
    ...
    def DeleteCache(self, package):
        return self.AdbLd(f"shell pm clear {package}")
    def OpenLink(self, link: str, package: str=""):
        self.AdbLd(f'shell am start -a android.intent.action.VIEW -d "{link}" {package}')
    def changeFolderPicture(self, path):
        with open(os.path.join(self.pathLD, 'vms\config\leidian%s.config'%self.NameOrId), encoding="utf-8") as file:
            # print(file.read())
            # data = json.loads(str(file.read()))
            data = file.read()
            pathOld = data.split('"statusSettings.sharedPictures": "')[1].split('"')[0]
            path = path.replace("\\", '/')
            data = data.replace(pathOld, path)
            file.close()
        with open(os.path.join(self.pathLD, 'vms\config\leidian%s.config'%self.NameOrId), "w", encoding="utf-8") as file:
            # json.dump(data, file)
            file.write(data)
            file.close()
    def OpenTikTokLite(self):
        # print("Luồng:", self.NameOrId)
    ...
    def ChangeInfo(self):
        ...
        self.ChangeProperty(f'--cpu 2 --memory 4096 --manufacturer {model.split("_")[0]} --model {model} --resolution 360,600,160 --imei auto --pnumber +"{random.choice(dauso)}{random.randint(1000000, 9999999)}" --androidid auto --mac auto --simserial auto --imsi auto')
        # string = ""
        # char = "qwertyuiopasdfghmcnvxbcz"
        # for i in range(10):
        #     string += char[random.randint(0, len(char)-1)]
        self.Rename("emulator-%s"%self.NameOrId)
    ...
    def Grant(self, package, permission):
        self.AdbLd("shell pm grant %s android.permission.%s"%(package, permission))
    ...
    def AdbLd(self, cmd):
        # return  str(subprocess.Popen(f"adb -s {self.local} {cmd}", creationflags=subprocess.CREATE_NO_WINDOW, stdout=subprocess.PIPE, shell=True, cwd=self.pathLD, start_new_session=True).stdout.read().decode("utf-8"))
        return str(self.ExecuteLD(f'adb --{self.param} {self.NameOrId} --command "{cmd}"').decode("utf-8"))
    def DownCPU(self, rate):
        self.ExecuteLD(f"downcpu --{self.param} {self.NameOrId} --rate {rate}")
    def IsDevice_Running(self):
        a = self.ExecuteLD(f"isrunning --{self.param} {self.NameOrId}").decode("utf-8")
        return "running"  in str(a)
    ...
    def GetDevices(self):
        list = str(self.ExecuteLD(f"list").decode("utf-8")).replace("b'","").replace("'","").replace('\n', '').split("\r")
        print(list)
        return list
    def GetAllLDPlayerRunning(self):
        allcommandlines = []
        allnames = []
        for p in psutil.process_iter():
            if p.name().lower() == "dnplayer.exe":
                    try:
                        allcommandlines.append(p.as_dict())
                    except Exception as fe:
                        pass
            allnames = [
                {'path': os.path.normpath(co["cmdline"][0]), 'index': str(co["cmdline"][1].strip('index=|'))}
    ...
    def GetDevices2(self):
        ...
        return Info_Devices
class CheckNode:
    def __init__(self, ld: LDPlayer) -> None:
        self.ld = ld
    def elementIsChecked(self, **kwargs):
        """
        This function checks if an element is checked in an XML file.
        :return: the name of the first element that is found to be checked (has a value of True) in the
        specified XML file. If none of the elements are found to be checked within the specified number
        of iterations (default is 30), the function does not return anything.
        """
        repeat = kwargs.get("repeat", 20)
        index = kwargs.get("index", 0)
        for i in range(repeat):
            _file = self.ld.DumXml()
            for name, value in kwargs.items():
                check, _ = self.ld.FindXml(value, _file, index=index)
                if check: return name
            time.sleep(1)
        return "notElement"
    def checkXML(self, **kwargs):
        xpath = kwargs.get("element", "")
        repeat = kwargs.get("repeat", 20)
        index = kwargs.get("index", 0)
        click = kwargs.get("click", True)
        for i in range(repeat):
            pos = self.ld.GetPosXml(xpath)
            if pos != []:
                x, y = pos[index]
                if click: self.ld.Click(x,y)
                return True
            time.sleep(1)
        return False
